...
(function($){
	...
	function syncFilterRules(target){
		...
		for(var i=0; i<fields.length; i++){
			var field = fields[i];
			var input = getFilterComponent(target, field);
			if (!input.length || !input[0].filter || !input[0].filterOptions){
				continue;
			}
			var filterOpts = input[0].filterOptions;
			if (!filterOpts.op || !filterOpts.op.length){
				continue;
			}
			var value = input[0].filter.getValue(input);
			if (filterOpts.options && filterOpts.options.prompt == value){
				value = '';
			}
	...
	$.extend($.fn.datagrid.methods, {
		...
		showFilterBar: function(jq){
			return jq.each(function(){
				...
				opts.showFilterBar = true;
				$(this).datagrid('getPanel').find('.datagrid-header .datagrid-filter-row').show();
				$(this).datagrid('resizeFilter');
		...
		hideFilterBar: function(jq){
			return jq.each(function(){
				var opts = $(this).datagrid('options');
				opts.showFilterBar = false;
				$(this).datagrid('getPanel').find('.datagrid-header .datagrid-filter-row').hide();
				$(this).datagrid('resize');
			});
		}
	});
